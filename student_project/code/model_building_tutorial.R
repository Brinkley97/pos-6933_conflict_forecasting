...
# Replace theory_name with your own theoretical model.
theory_name <- "Natural Resources Model"
theory_slug <- gsub("[^a-z0-9]+", "_", tolower(theory_name))
theory_slug <- gsub("^_|_$", "", theory_slug)

# Replace these predictors with the variables that best represent your theory.
theory_name <- "Natural Resources Model"

selected_predictors <- c(
  # WGI — Governance quality around resource management
  "wgi_CC_EST",          # Elite capture of resource rents
  "wgi_GE_EST",          # State capacity to manage extractive wealth
  "wgi_PV_EST",          # Resource-driven political instability
  "wgi_RL_EST",          # Legal governance of extraction
  "wgi_RQ_EST",          # Regulatory control of extractive sectors
  # V-Dem — Structural inequality from resource concentration
  "vdem_v2xeg_eqdr",     # Unequal distribution of resource wealth
  "vdem_v2xeg_eqaccess", # Exclusion from resource-based power
  "vdem_v2x_egal",       # Structural inequality
  # Freedom House
  "fh_cl",               # Civil liberties suppression under resource states
  # Polity — Regime context for resource governance
  "polity_polity2",      # Autocratic regimes mismanage resource rents
  "polity_autoc",        # Resource-funded authoritarianism
  "polity_xconst",       # Unconstrained executives capture resource wealth
  "polity_durable"       # Regime instability around extractive sectors
)
...
if (nzchar(env_horizons)) {
  ...
  if (length(parsed_horizons) > 0) {
    horizon_ids <- parsed_horizons
  }
}

env_rf_trees <- suppressWarnings(as.integer(Sys.getenv("MODEL_RF_TREES", unset = "")))
if (!is.na(env_rf_trees) && env_rf_trees > 0) {
  rf_num_trees <- env_rf_trees
}

env_cv_folds <- suppressWarnings(as.integer(Sys.getenv("MODEL_CV_FOLDS", unset = "")))
if (!is.na(env_cv_folds) && env_cv_folds > 1) {
  cv_folds <- env_cv_folds
}

env_valid_window <- suppressWarnings(as.integer(Sys.getenv("MODEL_VALID_WINDOW", unset = "")))
if (!is.na(env_valid_window) && env_valid_window > 0) {
  cv_valid_window <- env_valid_window
}

env_min_train <- suppressWarnings(as.integer(Sys.getenv("MODEL_MIN_TRAIN_MONTHS", unset = "")))
if (!is.na(env_min_train) && env_min_train > 0) {
  cv_min_train_months <- env_min_train
}
...
if (nzchar(env_gbm_trees)) {
  ...
  if (length(parsed_gbm_trees) > 0) {
    gbm_tree_values <- parsed_gbm_trees
  }
}
...
make_time_cv_folds <- function(train_data, n_folds, valid_window, min_train_months) {
  ...
  validation_starts <- validation_starts[validation_starts >= (min(all_month_ids) + min_train_months)]
  ...
  for (i in seq_along(validation_starts)) {
    ...
    train_rows <- which(train_data$month_id < valid_start)
    valid_rows <- which(train_data$month_id >= valid_start & train_data$month_id <= valid_end)

    fold_name <- paste0("Fold", i)
    train_index[[fold_name]] <- train_rows
    valid_index[[fold_name]] <- valid_rows
  }
...
build_horizon_split <- function(panel_data, horizon_id, predictor_cols, last_train, first_test, last_test) {
  ...
  test_rows <- panel_data %>%
    filter(month_id >= (first_test - horizon_id), month_id <= (last_test - horizon_id)) %>%
    mutate(
      horizon = horizon_id,
      origin_month_id = month_id,
      target_month_id = month_id + horizon_id
  ...
  train_ready <- train_rows %>% filter(if_all(all_of(required_cols), ~ !is.na(.x)))
  test_ready <- test_rows %>% filter(if_all(all_of(required_cols), ~ !is.na(.x)))

  list(train = train_ready, test = test_ready, outcome_name = outcome_name)
}
...
fit_theory_rf <- function(train_data, test_data, predictors, outcome_name, cv_object, num_trees = 150) {
...
fit_theory_gbm <- function(train_data, test_data, predictors, outcome_name, cv_object, tree_values) {
  model_formula <- as.formula(paste(outcome_name, "~", paste(predictors, collapse = " + ")))
  ctrl <- make_time_ctrl(cv_object)
...
compute_probabilistic_metrics <- function(prediction_table, tuning_table) {
  joined <- prediction_table %>%
    left_join(tuning_table %>% select(horizon, model, cv_rmse), by = c("horizon", "model")) %>%
...
save_model_outputs <- function(metric_table, prediction_table, tuning_table, importance_table, model_code) {
...
write_results_guidance <- function(
    output_path,
    theory_name,
    theory_slug,
    student_root,
    quick_mode,
    best_natural_resources_model,
    benchmark_best_model,
    natural_resources_win_summary) {
  lines <- c(
    ...
    "----------------------",
    paste("Best Natural Resources model by average RMSE:", best_natural_resources_model),
    paste("Best overall benchmark by average RMSE:", benchmark_best_model),
    ""
  )

  if (nrow(natural_resources_win_summary) > 0) {
    lines <- c(
      lines,
    ...
    for (i in seq_len(nrow(natural_resources_win_summary))) {
      row_i <- natural_resources_win_summary[i, ]
      lines <- c(
        lines,
        paste0(
...
required_inputs <- c(
  ...
  benchmark_lag_path,
  benchmark_roll_path,
  benchmark_lag_tuning_path,
  benchmark_roll_tuning_path
)
...
if (length(missing_predictors) > 0) {
  stop(
    "These selected predictors are not in the imputed dataset:\n",
...
# -----------------------------------------------------------------------------
# Fit the two Natural Resources models
# -----------------------------------------------------------------------------
...
gbm_metrics_list <- list()
gbm_predictions_list <- list()
gbm_tuning_list <- list()
gbm_importance_list <- list()
...
for (horizon_id in horizon_ids) {
  ...
  rf_importance_list[[as.character(horizon_id)]] <- extract_importance_table(
    model_obj = rf_fit$model,
    horizon_id = horizon_id,
    model_code = "natural_resources_rf"
  )

  gbm_fit <- fit_theory_gbm(
    train_data = train_ready,
    test_data = test_ready,
    predictors = selected_predictors,
    outcome_name = outcome_name,
    cv_object = cv_object,
    tree_values = gbm_tree_values
  )
  ...
  gbm_tuning_list[[as.character(horizon_id)]] <- tibble::tibble(
    ...
    best_mtry = NA_real_,
    splitrule = paste0(
      "gbm_depth_", gbm_best$interaction.depth[1],
  ...
  gbm_importance_list[[as.character(horizon_id)]] <- extract_importance_table(
    model_obj = gbm_fit$model,
    horizon_id = horizon_id,
    model_code = "natural_resources_gbm"
  )
}
...
save_model_outputs(rf_metric_table, rf_prediction_table, rf_tuning_table, rf_importance_table, "natural_resources_rf")
save_model_outputs(gbm_metric_table, gbm_prediction_table, gbm_tuning_table, gbm_importance_table, "natural_resources_gbm")

# -----------------------------------------------------------------------------
# Compare against the benchmark models
# -----------------------------------------------------------------------------

model_order_codes <- c("natural_resources_rf", "natural_resources_gbm", "benchmark_lag123", "benchmark_roll3")
key_columns <- c("horizon", "country_id", "month_id", "origin_month_id", "target_month_id")

all_predictions <- bind_rows(
  rf_prediction_table,
  gbm_prediction_table,
  benchmark_lag_predictions,
  benchmark_roll_predictions
) %>%
...
comparison_point_metrics <- matched_predictions %>%
  ...
  summarise(
    n = n(),
    mse = mean((actual_log - pred_log)^2),
    rmse = sqrt(mse),
    .groups = "drop"
  ) %>%
...
comparison_tuning_table <- bind_rows(
  ...
  benchmark_lag_tuning,
  benchmark_roll_tuning
)

comparison_prob_metrics <- compute_probabilistic_metrics(matched_predictions, comparison_tuning_table)

comparison_all_metrics <- comparison_point_metrics %>%
  left_join(comparison_det_crps %>% select(horizon, model, crps_det), by = c("horizon", "model")) %>%
  left_join(
    comparison_prob_metrics %>%
      select(horizon, model, crps_prob, coverage80, calibration_error80, sharpness_width80),
    by = c("horizon", "model")
  ) %>%
  arrange(horizon, rmse)
...
natural_resources_vs_benchmark_by_horizon <- comparison_long_metrics %>%
  filter(model %in% c("natural_resources_rf", "natural_resources_gbm")) %>%
  left_join(best_benchmark_by_horizon_metric, by = c("horizon", "metric")) %>%
  mutate(
    gap_to_best_benchmark = value - best_benchmark_value,
    improved_on_best_benchmark = gap_to_best_benchmark < 0
  ) %>%
  arrange(metric, horizon, value)

natural_resources_vs_benchmark_summary <- natural_resources_vs_benchmark_by_horizon %>%
  group_by(model, metric) %>%
  summarise(
    horizons_beating_best_benchmark = sum(improved_on_best_benchmark),
...
natural_resources_model_overview <- comparison_all_metrics %>%
  group_by(model) %>%
  summarise(
    horizons = n(),
    mean_mse = mean(mse),
...
natural_resources_importance_all <- bind_rows(rf_importance_table, gbm_importance_table) %>%
  group_by(model, horizon) %>%
  arrange(desc(importance), .by_group = TRUE) %>%
  mutate(
    importance_share = importance / sum(importance),
    rank_within_horizon = row_number()
  ) %>%
  ungroup()

natural_resources_feature_importance_summary <- natural_resources_importance_all %>%
...
readr::write_csv(matched_predictions, file.path(comparison_dir, "matched_predictions_all_models_log_dv.csv"))
readr::write_csv(comparison_point_metrics, file.path(comparison_dir, "comparison_metrics_log_dv.csv"))
readr::write_csv(comparison_det_crps, file.path(comparison_dir, "comparison_crps_det_log_dv.csv"))
readr::write_csv(comparison_prob_metrics, file.path(comparison_dir, "comparison_probabilistic_metrics_log_dv.csv"))
readr::write_csv(comparison_all_metrics, file.path(comparison_dir, "comparison_all_metrics_log_dv.csv"))
readr::write_csv(best_model_by_horizon_metric, file.path(comparison_dir, "best_model_by_horizon_metric_log_dv.csv"))
readr::write_csv(natural_resources_vs_benchmark_by_horizon, file.path(comparison_dir, "natural_resources_vs_best_benchmark_by_horizon.csv"))
readr::write_csv(natural_resources_vs_benchmark_summary, file.path(comparison_dir, "natural_resources_vs_benchmark_summary.csv"))
readr::write_csv(natural_resources_model_overview, file.path(comparison_dir, "natural_resources_model_overview.csv"))
readr::write_csv(natural_resources_feature_importance_summary, file.path(comparison_dir, "natural_resources_feature_importance_summary.csv"))
...
model_label_map <- c(
  ...
  natural_resources_rf = "Natural Resources Random Forest",
  natural_resources_gbm = "Natural Resources GBM"
)

model_color_map <- c(
  "Benchmark RF (Lag 1-3)" = "#0B5CAB",
  "Benchmark RF (Roll3)" = "#5C88C4",
  "Natural Resources Random Forest" = "#E87511",
  "Natural Resources GBM" = "#A33B20"
)
...
comparison_plot_table <- comparison_long_metrics %>%
  mutate(
    model_label = recode(model, !!!model_label_map),
...
metrics_plot <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = paste("Model Comparison:", theory_name),
    subtitle = "Natural Resources models compared against both benchmark models",
    x = "Forecast horizon",
    y = "Metric value",
    color = "Model"
  ) +
...
ggplot2::ggsave(
  file.path(plots_dir, "all_models_metrics_by_horizon.png"),
  metrics_plot,
  width = 12,
  height = 8,
  dpi = 320
...
ggplot2::ggsave(
  file.path(plots_dir, "calibration80_by_horizon.png"),
  calibration_plot,
  width = 11,
  height = 6.5,
  dpi = 320
...
sharpness_plot_data <- comparison_prob_metrics %>%
  mutate(model_label = recode(model, !!!model_label_map))

sharpness_plot <- ggplot2::ggplot(
  sharpness_plot_data,
  ggplot2::aes(x = horizon, y = sharpness_width80, color = model_label)
) +
  ggplot2::geom_point(size = 2.1) +
  ggplot2::scale_color_manual(values = model_color_map) +
  ggplot2::scale_x_continuous(breaks = sort(unique(horizon_ids))) +
  ggplot2::labs(
    title = paste("80% Interval Sharpness by Horizon:", theory_name),
    subtitle = "Lower values indicate narrower predictive intervals",
    x = "Forecast horizon",
    y = "Average 80% interval width",
    color = "Model"
  ) +
  project_plot_theme()

if (length(unique(sharpness_plot_data$horizon)) > 1L) {
  sharpness_plot <- sharpness_plot + ggplot2::geom_line(linewidth = 1.05)
}

ggplot2::ggsave(
  file.path(plots_dir, "sharpness80_by_horizon.png"),
  sharpness_plot,
  width = 11,
  height = 6.5,
  dpi = 320
...
importance_plot_data <- natural_resources_feature_importance_summary %>%
  group_by(model) %>%
...
importance_plot <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = paste("Top Natural Resources Indicators by Model:", theory_name),
    subtitle = "Average feature importance share across forecast horizons",
    x = NULL,
    y = "Average importance share"
  ) +
  project_plot_theme()

ggplot2::ggsave(
  file.path(plots_dir, "top_feature_importance.png"),
  importance_plot,
  width = 11,
  height = 7,
  dpi = 320
...
best_natural_resources_model <- natural_resources_model_overview %>%
  filter(model %in% c("natural_resources_rf", "natural_resources_gbm")) %>%
  slice_min(order_by = mean_rmse, n = 1, with_ties = FALSE) %>%
  pull(model)

benchmark_best_model <- natural_resources_model_overview %>%
  filter(model %in% c("benchmark_lag123", "benchmark_roll3")) %>%
  slice_min(order_by = mean_rmse, n = 1, with_ties = FALSE) %>%
  pull(model)
...
write_results_guidance(
  ...
  best_natural_resources_model = best_natural_resources_model,
  benchmark_best_model = benchmark_best_model,
  natural_resources_win_summary = natural_resources_vs_benchmark_summary
)
...
writeLines(
  c(
    ...
    "----------------------------",
    apply(natural_resources_model_overview, 1, function(row_i) {
      paste(
        row_i[["model"]],
