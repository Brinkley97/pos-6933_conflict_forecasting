# =============================================================================
# Week 5 - Step 3: Train Model 2 (Rolling average of last three months (roll3)
# =============================================================================
# DV used for each horizon:
...
out_dir <- file.path(week5_dir, "outputs", "week5", "model_rf_roll3")
...
for (h in horizons) {
  cat("\nTraining Model 2 for horizon ", h, "...\n", sep = "")
  ...
  needed_cols <- c("log_roll3", "month_id", target_log)
  ...
  set.seed(8000 + h)
  fit <- caret::train(
    form = as.formula(paste0(target_log, " ~ log_roll3")),
    data = train_model,
    method = "ranger",
    metric = "RMSE",
    trControl = ctrl,
  ...
  all_metrics[[as.character(h)]] <- metric_row(
    ...
    pred = pred_log,
    horizon = h,
    model_name = "rf_roll3_caret_cv"
  )

  all_predictions[[as.character(h)]] <- test_model %>%
    transmute(
      horizon = h,
      model = "rf_roll3_caret_cv",
      country_name,
      country_id,
      month,
      month_id,
  ...
  all_resamples[[as.character(h)]] <- fit$resample %>%
    mutate(horizon = h, model = "rf_roll3_caret_cv", .before = 1)
...
rmse_plot <- metrics_tbl %>%
  ...
  labs(
    title = "Model 2 (roll3): Test RMSE by Horizon",
...
mse_plot <- metrics_tbl %>%
  ...
  scale_x_continuous(breaks = horizons) +
  labs(
    title = "Model 2 (roll3): Test MSE by Horizon",
    x = "Horizon (months ahead)",
    y = "MSE (log DV)"
  ) +
  theme_minimal(base_size = 12)
